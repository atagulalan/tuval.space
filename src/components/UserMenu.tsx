...
export const UserMenu = ({ variant = 'default', hideCreateBoard = false }: UserMenuProps) => {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon" style={{ color: '#929bc9' }} className="rounded-full hover:opacity-80">
          {cachedPhotoURL ? (
            <img
              src={cachedPhotoURL}
              alt={user.username}
      ...
      >
        <DropdownMenuLabel className={cn(isBoardVariant ? 'text-white' : 'text-gray-100')}>
        ...
        <DropdownMenuSeparator className={cn(isBoardVariant ? 'bg-white/10' : 'bg-gray-700')} />
        {!hideCreateBoard && (
          <DropdownMenuItem 
            onSelect={(e) => {
              e.preventDefault();
