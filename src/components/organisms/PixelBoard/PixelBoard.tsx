...
export const PixelBoard = forwardRef<PixelBoardRef, PixelBoardProps>(
  (
    ...
    const [pendingPixels, setPendingPixels] = useState<
      Map<string, PendingPixel>
    >(new Map());
    const [history, setHistory] = useState<Map<string, PendingPixel>[]>([
      new Map(),
    ]);
    ...
    const isBoardLoaded =
      board.width > 0 && board.height > 0 && pixels.length > 0;
    ...
    const buildBaseCache = useCallback(() => {
      ...
      if (duration > 50) {
        console.warn(
          `[PixelBoard] buildBaseCache took ${duration.toFixed(2)}ms for ${board.width}x${board.height} board`
        );
      }
    ...
    const drawGrid = useCallback(() => {
      ...
      if (baseCacheNeedsUpdateRef.current) {
        ...
        if (cacheDuration > 100) {
          console.warn(
            `[PixelBoard] buildBaseCache in drawGrid took ${cacheDuration.toFixed(2)}ms`
          );
        }
      }
      ...
      if (offscreenCanvas) {
        ...
        ctx.drawImage(
          ...
          0,
          board.width,
          board.height, // Source: entire cache (1:1 scale)
          currentOffset.x,
          currentOffset.y, // Destination position
          board.width * currentZoom,
          board.height * currentZoom // Destination size (scaled)
        );
      }
      ...
      if (drawGridLines) {
        ...
        for (
          ...
          y <= Math.min(board.height, endY);
          y++
        ) {
          const screenY = y * currentZoom + currentOffset.y;
          const screenStartX =
            Math.max(0, startX) * currentZoom + currentOffset.x;
          const screenEndX =
            Math.min(board.width, endX) * currentZoom + currentOffset.x;
        ...
        for (
          ...
          x <= Math.min(board.width, endX);
          x++
        ) {
          const screenX = x * currentZoom + currentOffset.x;
          const screenStartY =
            Math.max(0, startY) * currentZoom + currentOffset.y;
          const screenEndY =
            Math.min(board.height, endY) * currentZoom + currentOffset.y;
    ...
    }, [board.height, board.width, isGuest, buildBaseCache]);
      ...
      const render = () => {
        ...
        if (pixelsRef.current.length === 0) {
          ...
          if (waitDuration > 1000 && renderCount % 60 === 0) {
            ...
            console.warn(
              `[PixelBoard] Still waiting for pixels to load... (${(waitDuration / 1000).toFixed(1)}s)`
            );
          }
        ...
        if (pixelsWaitStartTime !== null) {
          const waitDuration = performance.now() - pixelsWaitStartTime;
          console.log(
            `[PixelBoard] Pixels loaded after ${waitDuration.toFixed(2)}ms (${renderCount} frames waited)`
          );
        ...
        if (isAnimatingRef.current) {
          ...
          const currentZoom =
            ...
            (targetZoomRef.current - startZoomRef.current) * easedProgress;

          // Interpolate offset
          const currentOffset = {
            x:
              startOffsetRef.current.x +
              (targetOffsetRef.current.x - startOffsetRef.current.x) *
                easedProgress,
            y:
              startOffsetRef.current.y +
              (targetOffsetRef.current.y - startOffsetRef.current.y) *
                easedProgress,
          };
        ...
        if (needsRedrawRef.current) {
          ...
          if (firstRenderTime === null) {
            ...
            console.log(
              `[PixelBoard] First render completed in ${timeToFirstRender.toFixed(2)}ms (${renderCount} frames)`
            );
          }
          ...
          if (drawDuration > 16) {
            console.warn(
              `[PixelBoard] drawGrid took ${drawDuration.toFixed(2)}ms (target: <16ms for 60fps)`
            );
          }
        ...
        if (frameDuration > 16 && renderCount % 60 === 0) {
          ...
          console.warn(
            `[PixelBoard] Frame took ${frameDuration.toFixed(2)}ms (frame ${renderCount})`
          );
        }
    ...
    const animateZoom = useCallback(
      (targetZoom: number, targetOffset: { x: number; y: number }) => {
        const currentZoom = zoomRef.current;
        const currentOffset = offsetRef.current;
    ...
    const handlePixelPlace = useCallback(
      (x: number, y: number) => {
        ...
        const effectiveCurrentColor = pendingPixel
          ? pendingPixel.color
          : currentColor;
      ...
      [isGuest, pixels, toast, currentUserId, availableQuota, onQuotaExceeded]
    );
      ...
      if (
        ...
        requestAnimationFrame(() => {
          ...
          const fitZoom = Math.max(
            2,
            Math.min(50, Math.floor(Math.min(horizontalZoom, verticalZoom)))
          );
    ...
    return (
      <div ref={containerRef} className="absolute inset-0 bg-canvas-dark">
        ...
        <div
          className={`absolute bottom-6 left-4 bg-black/20 backdrop-blur-xl border border-white/10 rounded-2xl px-4 py-2 text-sm z-10 pointer-events-none transition-all duration-300 ease-out ${
            showInfoDisplay
        ...
        >
          <p className="font-mono" style={{ color: '#929bc9' }}>
            {zoom.toFixed(0)}x
            {hoveredCell && ` | (${hoveredCell.x + 1}, ${hoveredCell.y + 1})`}
          </p>
        </div>
        ...
        {isGuest && (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10 pointer-events-none">
            <div
              className="bg-yellow-400/20 backdrop-blur-xl border border-yellow-400/30 rounded-2xl px-4 py-2 text-sm"
              style={{ color: '#929bc9' }}
            >
