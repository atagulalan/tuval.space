...
import type { PixelBoardRef } from '@/components/organisms/PixelBoard';
...
import { PixelBoard } from '@/components/organisms/PixelBoard';
import {
  ColorPicker,
  COLOR_PALETTES,
} from '@/components/molecules/ColorPicker';
import { PixelQuotaDisplay } from '@/components/molecules/PixelQuotaDisplay';
import { ModificationHistory } from '@/components/organisms/ModificationHistory';
import { BoardControlPanel } from '@/components/organisms/BoardControlPanel';
...
export const BoardPage = () => {
  ...
  const calculateAreaCertainty = useCallback(
    (
      x: number,
      y: number,
      width: number,
      ...
      if (normalizedX < CORNER_SIZE && normalizedY > 1 - CORNER_SIZE) {
        ...
        certainties['bottom-left'] =
          1 - Math.sqrt(distX * distX + distY * distY);
      }
      ...
      if (normalizedX > 1 - CORNER_SIZE && normalizedY > 1 - CORNER_SIZE) {
        const distX = (1 - normalizedX) / CORNER_SIZE;
        const distY = (1 - normalizedY) / CORNER_SIZE;
        certainties['bottom-right'] =
          1 - Math.sqrt(distX * distX + distY * distY);
      }

      // Top edge (but not corners)
      if (
        normalizedY < OPEN_THRESHOLD &&
        normalizedX >= CORNER_SIZE &&
        normalizedX <= 1 - CORNER_SIZE
      ) {
      ...
      if (
        normalizedX < OPEN_THRESHOLD &&
        normalizedY >= CORNER_SIZE &&
        normalizedY <= 1 - CORNER_SIZE
      ) {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const certainties = calculateAreaCertainty(
        e.clientX,
        e.clientY,
        width,
        height
    ...
    const timeouts = menuTimeoutsRef.current;
    const openTimes = menuOpenTimesRef.current;
    return () => {
    ...
    const fetchBoard = async () => {
      ...
      try {
        ...
        if (boardData) {
          ...
          if (
            !boardData.isPublic &&
            (!user || user.uid !== boardData.ownerId)
          ) {
            toast({
              title: 'Access denied',
          ...
          logError(
            'Board Not Found',
            `No board found with name "${boardName}"`,
            'BoardPage'
          );
          toast({
        ...
        logError('Board Fetch Error', errorMessage, 'BoardPage', {
  ...
  return (
    <div ref={pageContainerRef} className="h-screen relative overflow-hidden">
      ...
      <PixelBoard
        ...
        availableQuota={
          user ? Math.max(0, user.pixelQuota - pendingPixelsNeedingQuota) : 0
        }
      ...
      <div className="absolute top-4 left-4 z-30 pointer-events-auto">
        <div
          className={`flex items-center gap-2 bg-black/20 backdrop-blur-xl border border-white/10 rounded-2xl px-2 py-2 transition-all duration-300 ease-out ${
            isFullscreen
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    style={{
                      color: '#929bc9',
                      borderColor: 'rgba(146, 155, 201, 0.2)',
                    }}
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-30 pointer-events-auto">
        <div
          className={`w-auto max-w-2xl bg-black/20 backdrop-blur-xl border border-white/10 rounded-2xl transition-all duration-300 ease-out ${
            isFullscreen
        ...
        >
          <ColorPicker
            ...
            colors={
              board.customPalette && board.customPalette.length > 0
                ? board.customPalette
          ...
          {!user && (
            <div className="mx-4 mb-4 p-3 bg-white/10 rounded-xl">
              <p
                ...
                style={{ color: '#929bc9', opacity: 0.7 }}
              >
            ...
            <div>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {board.maxPixels.toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between">
